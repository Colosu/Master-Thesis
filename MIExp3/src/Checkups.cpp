	...
	while (detected && !arcIter.Done()) {
		if (to_string(arcIter.Value().ilabel) == aux.front().getInput()) {
			if (to_string(arcIter.Value().olabel) == aux.front().getOutput()) {
				detected = checkMutation(transducer, aux, arcIter.Value().nextstate, size, i+1, ticks);
			}
		}
		arcIter.Next();
		ticks++;
...
bool Checkups::checkMutation(StdMutableFst* transducer, list<IOpair> aux, int state, int size, int i,  int &ticks) {
